...
@section('content')
        ...
        <div class="card">
            <div class="card-header">Tens a certeza que desejas apagar os dados de: <span class="font-weight-bold text-secondary">{{ $paciente->nome }}</span></div>
            <div class="card-body">
                <form action="{{ route('apagarPaciente', $paciente->id) }}}" method="post">
                    @csrf
                    <div class="row">
                        ...
                        <div class="form-group col-6">
                            <a class="btn btn-block btn-primary btn-sm text-center" href="{{ route('pacienteList') }}" dismiss><i class="fa fa-close"> Cancelar</i></a>
                        </div>
                    </div>
                </form>
            </div>
